...
// ─── SVG icon paths ───────────────────────────────────────────────────────────
// Each icon has an outline version (inactive) and filled version (active).
// Active: white center fill inside the outline shape.

function IconDashboard({ active }) {
  const stroke = active ? "white" : colors.slate;
  const fill   = active ? "white" : "none";
  return (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" strokeWidth="1.8" stroke={stroke} strokeLinecap="round" strokeLinejoin="round">
    ...
    <div style={{
      ...
      {TABS.map(({ key, label, Icon }) => {
        const active = key === "dashboard"
          ? (activeView === "dashboard" || activeView === "session")
          : key === "practice"
          ? activeView === "practice"
          : key === "glossary"
          ? activeView === "glossary"
        ...
        return (
          ...
          >
            {/* Active pill background — subtle, doesn't overpower */}
            {active && (
              <div style={{
                position:     "absolute",
                top:          6,
