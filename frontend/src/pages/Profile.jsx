      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: isMobile ? 24 : 32 }}>
        <button
          ...
          style={{
            ...
            borderRadius: 8, color: colors.slate, fontSize: 18,
        ...
        {editing ? (
          <>
            <input
              ...
              style={{ width: "100%", padding: "12px 14px", background: "#1e293b", border: `1px solid ${saveError ? colors.red : colors.indigo}`, borderRadius: 8, color: colors.text, fontSize: 15, outline: "none", fontFamily: "inherit", boxSizing: "border-box", marginBottom: 8 }}
            />
            {saveError && <p style={{ color: colors.red, fontSize: 13, margin: "0 0 10px" }}>⚠️ {saveError}</p>}
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={handleSaveUsername} disabled={savingName} style={{ ...sharedStyles.btn, ...sharedStyles.btnPrimary, fontSize: 13, padding: "9px 18px" }}>
                {savingName ? "Saving..." : "Save"}
              </button>
              <button onClick={() => { setEditing(false); setEditUsername(username); setSaveError(""); }} style={{ ...sharedStyles.btn, ...sharedStyles.btnSecondary, fontSize: 13, padding: "9px 18px" }}>
                Cancel
              </button>
            </div>
          </>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <button onClick={() => { setEditing(true); setSaveError(""); }} style={{ ...sharedStyles.btn, ...sharedStyles.btnSecondary, fontSize: 13, padding: "7px 14px" }}>
              Edit
            </button>
          </div>
        )}
